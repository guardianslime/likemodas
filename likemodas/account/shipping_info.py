...
def address_form() -> rx.Component:
    ...
    return rx.form(
        rx.vstack(
            ...
            # Usamos un Grid que se adapta a móvil
            rx.grid(
                ...
                rx.vstack(
                    rx.text("Ciudad*"),
                    searchable_select(placeholder="Selecciona una ciudad...", options=AppState.cities, on_change_select=AppState.set_city, value_select=AppState.city, search_value=AppState.search_city, on_change_search=AppState.set_search_city, filter_name="shipping_city_filter"),
                    spacing="1", align_items="stretch"
                ),
                rx.vstack(
                    rx.text("Barrio"),
                    searchable_select(placeholder="Selecciona un barrio...", options=AppState.neighborhoods, on_change_select=AppState.set_neighborhood, value_select=AppState.neighborhood, search_value=AppState.search_neighborhood, on_change_search=AppState.set_search_neighborhood, filter_name="shipping_neighborhood_filter", is_disabled=~AppState.neighborhoods),
                    spacing="1", align_items="stretch"
                ),
                rx.vstack(
                    rx.text("Dirección* (Calle, Carrera, Apto, etc.)"),
                    rx.input(name="address", required=True),
                    spacing="1", align_items="stretch",
                    # En escritorio ocupa 2 columnas, en móvil 1
                    grid_column={"initial": "span 1", "md": "span 2"},
                ),
                columns={"initial": "1", "md": "2"}, # <-- La clave de la responsividad
                spacing="4", width="100%",
            ),
            rx.hstack(
                rx.button("Cancelar", on_click=AppState.toggle_form, color_scheme="gray", variant="soft"),
                rx.button("Guardar Dirección", type="submit", color_scheme="violet"),
                justify="end", width="100%", margin_top="1.5em",
            ),
            spacing="5", width="100%",
        ),
...
def address_card(address: ShippingAddressModel) -> rx.Component:
    """Componente rediseñado para mostrar una dirección guardada."""
    return rx.card(
        rx.vstack(
            rx.hstack(
            ...
            rx.hstack(
                ...
                rx.button(
                    ...
                    variant="outline", size="1", color_scheme="violet"
                ),
            ...
            rx.vstack(
                rx.cond(
                    ...
                    rx.grid(
                        ...
                        columns={"initial": "1", "md": "2"}, # Grid responsivo para las tarjetas
                        spacing="5",
                        width="100%",
                    ),
                    rx.center(
