...
from .pages import search_results, about, pricing, dashboard, landing, test, category
from .blog import (
    page as blog_page,
...
# --- ✅ CAMBIO 1: Se importa el layout base para usarlo como wrapper ---
...
app = rx.App(
    theme=rx.theme(
        ...
        # --- ✅ CAMBIO 2: Se elimina 'scaling' para evitar saltos de tamaño ---
        radius="medium",
        accent_color="sky"
    ),
)

# --- ✅ CAMBIO 3: Todas las páginas ahora se envuelven en base_page ---
# --- Definición de Rutas de la Aplicación ---
app.add_page(base_page(index_content()), on_load=HomePageState.on_load_main)
app.add_page(base_page(dashboard.dashboard_content()), route="/dashboard", title="Dashboard", on_load=cart_state.CartState.on_load)
app.add_page(base_page(category.category_content()), route="/category/[cat_name]", title="Categoría", on_load=cart_state.CartState.load_posts_and_set_category)
...
app.add_page(base_page(about.about_page_content()), route=navigation.routes.ABOUT_US_ROUTE)
app.add_page(base_page(pricing.pricing_page_content()), route=navigation.routes.PRICING_ROUTE)
...
app.add_page(base_page(blog_add.blog_post_add_page_content()), route=navigation.routes.BLOG_POST_ADD_ROUTE)
app.add_page(base_page(blog_edit.blog_post_edit_page_content()), route="/blog/[blog_id]/edit", on_load=blog_state.BlogEditFormState.on_load_edit)
...
app.add_page(base_page(purchases_page.purchase_history_page_content()), route="/my-purchases", title="Mis Compras", on_load=purchases_state.PurchaseHistoryState.load_purchases)
app.add_page(base_page(account_page_module.my_account_redirect_page_content()), route=navigation.routes.MY_ACCOUNT_ROUTE, on_load=rx.redirect(navigation.routes.SHIPPING_INFO_ROUTE))
app.add_page(base_page(shipping_info_module.shipping_info_page_content()), route=navigation.routes.SHIPPING_INFO_ROUTE, title="Información de Envío", on_load=shipping_info_state.ShippingInfoState.load_addresses)

# --- Rutas de Administración ---
app.add_page(base_page(admin_page.admin_confirm_page_content()), route="/admin/confirm-payments", title="Confirmar Pagos", on_load=admin_state.AdminConfirmState.load_pending_purchases)
app.add_page(base_page(admin_page.payment_history_page_content()), route="/admin/payment-history", title="Historial de Pagos", on_load=admin_state.PaymentHistoryState.load_confirmed_purchases)
app.add_page(base_page(contact_page.contact_entries_list_page_content()), route=navigation.routes.CONTACT_ENTRIES_ROUTE, on_load=contact_state.ContactState.load_entries)