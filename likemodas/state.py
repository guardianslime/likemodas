...
class AppState(reflex_local_auth.LocalAuthState):
    ...
    def _decode_qr_from_image(self, image_bytes: bytes) -> str:
        """
        Intenta decodificar el QR usando Pyzbar (Robustez) y luego OpenCV (Respaldo).
        """
        import io
        import cv2
        import numpy as np
        ...
        # 1. INTENTO CON PYZBAR (El mejor para QRs)
        try:
            # Importamos aquí adentro para no afectar el resto de la app
            from pyzbar.pyzbar import decode
            ...
            if decoded_objects:
                # Retornamos el contenido del primer QR encontrado
                result = decoded_objects[0].data.decode("utf-8")
                print(f"QR Detectado con Pyzbar: {result}")
                return result
                
        except Exception as e:
            print(f"Aviso: Pyzbar no pudo leer, intentando OpenCV. Error: {e}")

        # 2. INTENTO CON OPENCV (Plan B)
        try:
            # Convertimos bytes a array de numpy para OpenCV
            ...
            # Usamos el detector de OpenCV
            detector = cv2.QRCodeDetector()
            data, bbox, _ = detector.detectAndDecode(cv_img)

            if data:
                print(f"QR Detectado con OpenCV: {data}")
                return data
                
        except Exception as e:
            print(f"Error fatal en lectura OpenCV: {e}")

        # Si llegamos aquí, ninguno pudo leerlo
        print("No se encontró ningún código QR legible.")
        return ""
    ...
    async def show_product_detail(self, product_id: int):
        """
        Muestra el detalle financiero de un producto, corrigiendo el JOIN vacío
        y el cálculo falso del envío.
        """
        if not (self.is_admin or self.is_vendedor or self.is_empleado):
            yield rx.redirect("/")
            return
        ...
        with rx.session() as session:
            ...
            # ✨ CORRECCIÓN DE JOIN POR '.any()' PARA EVITAR DATOS VACÍOS ✨
            completed_purchases = session.exec(
                ...
                .where(
                    PurchaseModel.items.any(PurchaseItemModel.blog_post_id == product_id)
                )
            ).unique().all()
    ...
    def _calculate_finance_data(self):
        ...
        y calculando el envío sin falsos positivos.
        """
        self.is_loading = True
        yield
    ...
    def recalculate_all_shipping_costs(self):
        """
        Recalcula costos y visibilidad de etiquetas usando normalización y lógica optimista.
        """
        if not self._raw_posts:
            self.posts = []
            return

        # Si no hay dirección, variables vacías (lógica Optimista se activará)
        buyer_city_norm = ""
        buyer_barrio = None
        ...
        with rx.session() as session:
            # Cargar info de vendedores en bloque para eficiencia
            ...
            for post in self._raw_posts:
                ...
                moda_cities = seller.moda_completa_cities or []
                combined_cities = seller.combined_shipping_cities or []
                ...
                if is_moda and moda_cities and buyer_city_norm:
                ...
                # Tooltip Moda
                if is_moda:
                    threshold = post.free_shipping_threshold or 0
                    updated_post.moda_completa_tooltip_text = f"Envío gratis en compras sobre {format_to_cop(threshold)}"
                else:
                    updated_post.moda_completa_tooltip_text = ""

                # --- 📦 LÓGICA 2: ENVÍO COMBINADO (CORREGIDA) ---
                is_combined = post.combines_shipping
                
                # Solo filtramos si hay restricción Y sabemos dónde está el comprador
                if is_combined and combined_cities and buyer_city_norm:
                    # Normalizamos para evitar errores de tildes (Bogotá vs Bogota)
                ...
                # Tooltip Combinado
                if is_combined:
                    limit = post.shipping_combination_limit or 1
                    updated_post.envio_combinado_tooltip_text = f"Combina hasta {limit} productos."
                else:
    ...
    # --- VERSIÓN CORREGIDA: Cambiado 'product_id' por 'post_id' ---
    @rx.event
            ...
            if self.is_authenticated:
                ...
                if purchase_count > 0:
                    ...
                    if not user_original_comment:
                        ...
                        if len(user_original_comment.updates) < 3:
                            self.show_review_form = True
...
# 3. --- Añadir los nuevos setters y event handlers a AppState ---
    ...
    # ==========================================
    # SISTEMA DE REPORTES (LOGICA QUE FALTABA)
    # ==========================================

    # Variables de estado para el modal de reportes
    show_report_modal: bool = False
    report_target_type: str = "" # "post" o "comment"
    report_target_id: int = 0
    report_reason: str = ""
    
    # Lista de reportes para el Admin
    admin_reports_list: list[ReportData] = []

    def open_report_modal(self, type: str, id: int):
        """Abre el modal para reportar contenido."""
        if not self.is_authenticated:
            return rx.toast.error("Debes iniciar sesión para reportar.")
        
        self.report_target_type = type
        self.report_target_id = id
        self.report_reason = "" # Limpiar selección previa
        self.show_report_modal = True

    def set_show_report_modal(self, value: bool):
        self.show_report_modal = value

    def set_report_reason(self, reason: str):
        self.report_reason = reason

    @rx.event
    def submit_report(self):
        """Envía el reporte a la base de datos."""
        if not self.report_reason:
            return rx.toast.error("Por favor selecciona un motivo.")
            
        with rx.session() as session:
            # Crear el reporte
            new_report = ReportModel(
                reporter_id=self.authenticated_user_info.id,
                reason=self.report_reason,
                status=ReportStatus.PENDING,
                # Asigna el ID al campo correcto según el tipo
                blog_post_id=self.report_target_id if self.report_target_type == "post" else None,
                comment_id=self.report_target_id if self.report_target_type == "comment" else None
            )
            ...
            for admin in admins:
                notif = NotificationModel(
                    ...
                    message=f"⚠️ Nuevo Reporte de Contenido: {self.report_reason}",
                    url="/admin/reports" # Asegúrate de tener esta ruta o cámbiala a donde gestiones reportes
                )
                session.add(notif)
        ...
        return rx.toast.success("Reporte enviado. Gracias por ayudar a la comunidad.")
    ...
    def resolve_report(self, report_id: int, action: str):
        """Acciones del admin: Descartar reporte o Eliminar contenido (Ban)."""
        with rx.session() as session:
            ...
            if action == "dismiss":
                ...
                # Lógica para ocultar post o borrar comentario
                if report.blog_post_id:
                    post = session.get(BlogPostModel, report.blog_post_id)
                    if post: 
                        post.publish_active = False # Ocultar producto
                elif report.comment_id:
                    comment = session.get(CommentModel, report.comment_id)
                    if comment: 
                        session.delete(comment) # Borrar comentario
                
                yield rx.toast.success("Contenido eliminado/oculto por moderación.")
