...
class PurchaseItemCardData(rx.Base):
    id: int; title: str; image_url: str; price_at_purchase_cop: str; quantity: int
...
class CommentData(rx.Base):
    id: int; content: str; rating: int; author_username: str
    author_initial: str; created_at_formatted: str
    updates: List["CommentData"] = []

class InvoiceItemData(rx.Base):
    name: str; quantity: int; price_cop: str; subtotal_cop: str
    iva_cop: str; total_con_iva_cop: str

class InvoiceData(rx.Base):
    id: int; purchase_date_formatted: str; status: str
    items: list[InvoiceItemData]; customer_name: str; customer_email: str
    shipping_full_address: str; shipping_phone: str; subtotal_cop: str
    iva_cop: str; total_price_cop: str
...
class AppState(reflex_local_auth.LocalAuthState):
    ...
    def authenticated_user_info(self) -> UserInfo | None:
        if not self.is_authenticated or self.authenticated_user.id < 0: return None
        with rx.session() as session:
            query = (sqlmodel.select(UserInfo).options(sqlalchemy.orm.joinedload(UserInfo.user)).where(UserInfo.user_id == self.authenticated_user.id))
    ...
    def handle_registration_email(self, form_data: dict):
        ...
        username = form_data.get("username"); email = form_data.get("email")
        password = form_data.get("password"); confirm_password = form_data.get("confirm_password")
        if not all([username, email, password, confirm_password]): self.error_message = "Todos los campos son obligatorios."; return
        if password != confirm_password: self.error_message = "Las contraseñas no coinciden."; return
        password_errors = validate_password(password)
        if password_errors: self.error_message = "\n".join(password_errors); return
        try:
            with rx.session() as session:
                if session.exec(sqlmodel.select(LocalUser).where(LocalUser.username == username)).first(): self.error_message = "El nombre de usuario ya está en uso."; return
                if session.exec(sqlmodel.select(UserInfo).where(UserInfo.email == email)).first(): self.error_message = "El email ya está registrado."; return
                ...
                self.success = True
        except Exception as e: self.error_message = f"Error inesperado: {e}"
    ...
    def verify_token(self):
        ...
        if not token: self.message = "Error: No se proporcionó un token de verificación."; return
        with rx.session() as session:
    ...
    def handle_forgot_password(self, form_data: dict):
        ...
        if not email: self.message, self.is_success = "Por favor, introduce tu correo electrónico.", False; return
        with rx.session() as session:
    ...
    def on_load_check_token(self):
        ...
        if not self.token: self.message, self.is_token_valid = "Enlace no válido. Falta el token.", False; return
        with rx.session() as session:
            db_token = session.exec(sqlmodel.select(PasswordResetToken).where(PasswordResetToken.token == self.token)).one_or_none()
            if db_token and db_token.expires_at.replace(tzinfo=timezone.utc) > datetime.now(timezone.utc): self.is_token_valid = True
            else:
    ...
    attr_tallas_ropa: list[str] = []; attr_numeros_calzado: list[str] = []
    attr_tamanos_mochila: list[str] = []; attr_colores: list[str] = []
    attr_material: str = ""; attr_tipo: str = ""
    search_attr_tipo: str = ""; search_attr_color: str = ""
    search_attr_talla_ropa: str = ""; search_attr_material: str = ""
    search_attr_numero_calzado: str = ""; search_attr_tamano_mochila: str = ""
    ...
    def set_search_attr_tipo(self, query: str): self.search_attr_tipo = query
    ...
    def set_price_str(self, price: str): self.price_str = price

    def _clear_add_form(self):
        self.title = ""; self.content = ""; self.price = ""; self.category = ""
        self.temp_images = []; self.attr_colores = []; self.attr_tallas_ropa = []
        self.attr_material = ""; self.attr_numeros_calzado = []; self.attr_tamanos_mochila = []
        self.attr_tipo = ""

    @rx.event
    def submit_and_publish(self, form_data: dict):
        if not self.is_admin: return rx.toast.error("Acción no permitida.")
        if not all([form_data.get("title"), form_data.get("price"), form_data.get("category")]): return rx.toast.error("Título, precio y categoría son obligatorios.")
        attributes = {}; category = form_data.get("category")
        ...
        with rx.session() as session:
            new_post = BlogPostModel(
                userinfo_id=self.authenticated_user_info.id, title=form_data["title"],
                content=form_data.get("content", ""), price=float(form_data.get("price", 0.0)),
                category=category, image_urls=self.temp_images, attributes=attributes,
                publish_active=True, publish_date=datetime.now(timezone.utc),
            )
    ...
    def start_editing_post(self, post_id: int):
        ...
        with rx.session() as session:
            ...
            if db_post and (db_post.userinfo_id == self.authenticated_user_info.id or self.is_admin):
                self.post_to_edit = db_post; self.post_title = db_post.title
                self.post_content = db_post.content; self.price_str = str(db_post.price or 0.0)
                self.post_images_in_form = db_post.image_urls.copy() if db_post.image_urls else []
                self.is_editing_post = True
            else: yield rx.toast.error("No se encontró la publicación o no tienes permisos.")

    @rx.event
    def cancel_editing_post(self, open_state: bool):
        if not open_state:
            self.is_editing_post = False; self.post_to_edit = None
            self.post_title = ""; self.post_content = ""; self.price_str = ""
            self.post_images_in_form = []

    @rx.event
    def save_edited_post(self, form_data: dict):
        if not self.is_admin or not self.post_to_edit: return rx.toast.error("No se pudo guardar la publicación.")
        with rx.session() as session:
            ...
            if post_to_update:
                ...
                except (ValueError, TypeError): return rx.toast.error("El precio debe ser un número válido.")
    ...
    def remove_temp_image(self, filename: str):
        if filename in self.temp_images: self.temp_images.remove(filename)
    ...
    def remove_edited_image(self, filename: str):
        if filename in self.post_images_in_form: self.post_images_in_form.remove(filename)
    ...
    def add_to_cart(self, post_id: int):
        ...
        self.show_detail_modal = False; self.product_in_modal = None
    ...
    def add_new_address(self, form_data: dict):
        if not all([form_data.get("name"), form_data.get("phone"), self.city, form_data.get("address")]): return rx.toast.error("Por favor, completa todos los campos requeridos.")
        latitude, longitude = None, None
        try:
            if self.manual_latitude and self.manual_longitude:
                latitude = float(self.manual_latitude); longitude = float(self.manual_longitude)
        except ValueError: return rx.toast.error("Las coordenadas deben ser números válidos.")
        with rx.session() as session:
        ...
        self.manual_latitude = ""; self.manual_longitude = ""; self.show_form = False
    ...
    def set_as_default(self, address_id: int):
        ...
        with rx.session() as session:
            ...
            if current_default: current_default.is_default = False; session.add(current_default)
            ...
            if new_default and new_default.userinfo_id == self.authenticated_user_info.id:
                new_default.is_default = True; session.add(new_default)
                session.commit()
                yield self.load_addresses()
            ...
            for post_id, quantity in self.cart.items():
                if post_id in post_map: 
                    session.add(PurchaseItemModel(
                        purchase_id=new_purchase.id, blog_post_id=post_map[post_id].id,
                        quantity=quantity, price_at_purchase=post_map[post_id].price
                    ))
            session.commit()
    ...
    filter_colors: list[str] = []; filter_tallas: list[str] = []
    filter_numeros_calzado: list[str] = []; filter_materiales_tela: list[str] = []
    ...
    def add_filter_value(self, filter_name: str, value: str):
        ...
        if value not in current_list:
            if len(current_list) >= 5: return rx.toast.info("Puedes seleccionar un máximo de 5 filtros por característica.")
            current_list.append(value); setattr(self, filter_name, current_list)

    def remove_filter_value(self, filter_name: str, value: str):
        current_list = getattr(self, filter_name)
        if value in current_list: current_list.remove(value); setattr(self, filter_name, current_list)

    def clear_all_filters(self):
        self.min_price = ""; self.max_price = ""
        self.filter_colors = []; self.filter_tallas = []
        self.filter_tipos_general = []; self.filter_materiales_tela = []
    ...
    def confirm_payment(self, purchase_id: int):
        ...
        with rx.session() as session:
            ...
            else: yield rx.toast.error("La compra no se encontró o ya fue confirmada.")
    ...
    def filtered_user_purchases(self) -> list[UserPurchaseHistoryCardData]:
        ...
        return [p for p in self.user_purchases if q in f"#{p.id}" or any(q in item.title.lower() for item in p.items)]
                        ...
                        if item.blog_post:
                            purchase_items_data.append(
                                PurchaseItemCardData(
                                    ...
                                    price_at_purchase_cop=format_to_cop(item.price_at_purchase), quantity=item.quantity
                                )
                            )
                temp_purchases.append(
                    UserPurchaseHistoryCardData(
                        id=p.id, purchase_date_formatted=p.purchase_date_formatted, status=p.status.value, total_price_cop=p.total_price_cop,
                        shipping_name=p.shipping_name, shipping_address=p.shipping_address, shipping_neighborhood=p.shipping_neighborhood, 
                        shipping_city=p.shipping_city, shipping_phone=p.shipping_phone, items=purchase_items_data
                    )
                )
    ...
    def load_notifications(self):
        ...
        with rx.session() as session:
            self.notifications = session.exec(sqlmodel.select(NotificationModel).where(NotificationModel.userinfo_id == self.authenticated_user_info.id).order_by(sqlmodel.col(NotificationModel.created_at).desc())).all()
    ...
    async def handle_contact_submit(self, form_data: dict):
        ...
        with rx.session() as session:
            ...
            entry = ContactEntryModel(
                first_name=form_data.get("first_name"), last_name=form_data.get("last_name"),
                email=form_data.get("email"), message=form_data.get("message"),
    ...
    def load_all_users(self):
        if not self.is_admin: self.all_users = []; return rx.redirect("/")
        with rx.session() as session:
            self.all_users = session.exec(sqlmodel.select(UserInfo).options(sqlalchemy.orm.joinedload(UserInfo.user))).all()

    @rx.event
    def toggle_admin_role(self, userinfo_id: int):
        if not self.is_admin: return rx.toast.error("No tienes permisos.")
        with rx.session() as session:
            user_info = session.get(UserInfo, userinfo_id)
            if user_info:
                if user_info.id == self.authenticated_user_info.id: return rx.toast.warning("No puedes cambiar tu propio rol.")
                user_info.role = UserRole.CUSTOMER if user_info.role == UserRole.ADMIN else UserRole.ADMIN
    ...
    def ban_user(self, userinfo_id: int, days: int = 7):
        ...
        with rx.session() as session:
            user_info = session.get(UserInfo, userinfo_id)
            if user_info:
                if user_info.id == self.authenticated_user_info.id: return rx.toast.warning("No puedes vetarte a ti mismo.")
    ...
    def unban_user(self, userinfo_id: int):
        ...
        with rx.session() as session:
            user_info = session.get(UserInfo, userinfo_id)
            if user_info:
                user_info.is_banned = False; user_info.ban_expires_at = None
                session.add(user_info); session.commit()
        return self.load_all_users()
    
    # --- Carga de Páginas y Contenido ---
    @rx.event
    def on_load_admin_store(self):
        if not self.is_admin: return rx.redirect("/")
        with rx.session() as session:
            results = session.exec(sqlmodel.select(BlogPostModel).options(sqlalchemy.orm.joinedload(BlogPostModel.comments)).where(BlogPostModel.userinfo_id == self.authenticated_user_info.id).order_by(BlogPostModel.created_at.desc())).unique().all()
            self.admin_store_posts = [ProductCardData.from_orm(p) for p in results]

    @rx.event
    def on_load_main_page(self):
        if self.is_admin: return rx.redirect("/admin/store")
        yield AppState.on_load
        full_url = self.router.url; product_id = None
        if full_url and "?" in full_url:
            parsed_url = urlparse(full_url); query_params = parse_qs(parsed_url.query)
            product_id_list = query_params.get("product")
            if product_id_list: product_id = product_id_list[0]
    ...
    review_rating: int = 0; review_content: str = ""
    show_review_form: bool = False; review_limit_reached: bool = False
    expanded_comments: dict[int, bool] = {}

    def set_review_rating(self, rating: int): self.review_rating = rating
    def toggle_comment_updates(self, comment_id: int): self.expanded_comments[comment_id] = not self.expanded_comments.get(comment_id, False)

    @rx.event
    def submit_review(self, form_data: dict):
        if not self.is_authenticated or not self.product_in_modal: return rx.toast.error("Debes iniciar sesión para opinar.")
        if self.review_rating == 0: return rx.toast.error("Debes seleccionar una valoración.")
        content = form_data.get("review_content", "")
        with rx.session() as session:
            user_info = self.authenticated_user_info
            if not user_info or not user_info.user: return rx.toast.error("No se pudo identificar al usuario.")
            if self.my_review_for_product:
                original_comment = session.get(CommentModel, self.my_review_for_product.id)
                if not original_comment: return rx.toast.error("El comentario que intentas actualizar ya no existe.")
                while original_comment.parent: original_comment = original_comment.parent
                if len(original_comment.updates) >= 2: return rx.toast.error("Ya has alcanzado el límite de 2 actualizaciones para esta compra.")
                new_update = CommentModel(
                    userinfo_id=user_info.id, blog_post_id=self.product_in_modal.id, rating=self.review_rating,
                    content=content, author_username=user_info.user.username, author_initial=user_info.user.username[0].upper(),
                    parent_comment_id=original_comment.id, purchase_item_id=original_comment.purchase_item_id 
                )
                session.add(new_update); yield rx.toast.success("¡Opinión actualizada!")
            else:
                unclaimed_purchase_item = self._find_unclaimed_purchase(session)
                if not unclaimed_purchase_item: return rx.toast.error("Debes comprar este producto para poder dejar una opinión.")
                new_review = CommentModel(
                    userinfo_id=user_info.id, blog_post_id=self.product_in_modal.id, rating=self.review_rating,
                    content=content, author_username=user_info.user.username, author_initial=user_info.user.username[0].upper(),
                    purchase_item_id=unclaimed_purchase_item.id
                )
                session.add(new_review); yield rx.toast.success("¡Gracias por tu opinión!")
            session.commit()
    ...
    def is_current_post_saved(self) -> bool:
        if not self.product_in_modal or not self.is_authenticated: return False
        return self.product_in_modal.id in self.saved_post_ids

    @rx.event
    def load_saved_post_ids(self):
        if not self.authenticated_user_info: self.saved_post_ids = set(); return
        with rx.session() as session:
            user_info = session.get(UserInfo, self.authenticated_user_info.id)
            if user_info: self.saved_post_ids = {post.id for post in user_info.saved_posts}

    @rx.event
    def toggle_save_post(self):
        if not self.authenticated_user_info or not self.product_in_modal: return rx.toast.error("Debes iniciar sesión para guardar publicaciones.")
        post_id = self.product_in_modal.id
        with rx.session() as session:
            user_info = session.get(UserInfo, self.authenticated_user_info.id)
            post_to_toggle = session.get(BlogPostModel, post_id)
            if not user_info or not post_to_toggle: return rx.toast.error("Error al procesar la solicitud.")
            if post_id in self.saved_post_ids:
                user_info.saved_posts.remove(post_to_toggle); self.saved_post_ids.remove(post_id)
                yield rx.toast.info("Publicación eliminada de tus guardados.")
            else:
                user_info.saved_posts.append(post_to_toggle); self.saved_post_ids.add(post_id)
    ...
    def on_load_saved_posts_page(self):
        ...
        if not self.authenticated_user_info:
            self.saved_posts_gallery = []; self.is_loading = False
            return rx.redirect(reflex_local_auth.routes.LOGIN_ROUTE)
        with rx.session() as session:
            user_info = session.get(UserInfo, self.authenticated_user_info.id)
            if user_info:
                ...
                for p in sorted_posts:
                    product_dto = ProductCardData.from_orm(p); product_dto.price_cop = p.price_cop
    ...
    def _convert_comment_to_dto(self, comment_model: CommentModel) -> CommentData:
        return CommentData(
            id=comment_model.id, content=comment_model.content, rating=comment_model.rating,
            author_username=comment_model.author_username, author_initial=comment_model.author_initial,
    ...
    def open_product_detail_modal(self, post_id: int):
        self.product_in_modal = None; self.show_detail_modal = True; self.current_image_index = 0
        self.product_comments = []; self.my_review_for_product = None
        self.review_rating = 0; self.review_content = ""; self.show_review_form = False; self.review_limit_reached = False
        with rx.session() as session:
            db_post = session.exec(sqlmodel.select(BlogPostModel).options(sqlalchemy.orm.joinedload(BlogPostModel.comments).joinedload(CommentModel.userinfo).joinedload(UserInfo.user), sqlalchemy.orm.joinedload(BlogPostModel.comments).joinedload(CommentModel.updates).joinedload(CommentModel.userinfo).joinedload(UserInfo.user)).where(BlogPostModel.id == post_id)).unique().one_or_none()
            if db_post and db_post.publish_active:
                ...
                if db_post.userinfo and db_post.userinfo.user:
                    product_dto.seller_name = db_post.userinfo.user.username; product_dto.seller_id = db_post.userinfo.id
                ...
                if self.is_authenticated:
                    user_info = self.authenticated_user_info
                    purchase_count = session.exec(sqlmodel.select(sqlmodel.func.count(PurchaseItemModel.id)).join(PurchaseModel).where(PurchaseModel.userinfo_id == user_info.id, PurchaseItemModel.blog_post_id == post_id, PurchaseModel.status.in_([PurchaseStatus.CONFIRMED, PurchaseStatus.SHIPPED]))).one()
                    if purchase_count > 0:
                        user_original_comment = next((c for c in db_post.comments if c.userinfo_id == user_info.id and c.parent_comment_id is None), None)
                        if not user_original_comment: self.show_review_form = True
                        else:
                            total_allowed_updates = purchase_count; current_updates_count = len(user_original_comment.updates)
                            if current_updates_count < total_allowed_updates:
                                self.show_review_form = True
                                latest_entry_in_thread = sorted([user_original_comment] + user_original_comment.updates, key=lambda c: c.created_at, reverse=True)[0]
                                self.my_review_for_product = self._convert_comment_to_dto(latest_entry_in_thread)
                                self.review_rating = latest_entry_in_thread.rating; self.review_content = latest_entry_in_thread.content
                            else: self.review_limit_reached = True
            else:
                self.show_detail_modal = False; yield rx.toast.error("Producto no encontrado o no disponible."); return
    ...
    def on_load_seller_page(self):
        self.is_loading = True; self.seller_page_info = None; self.seller_page_posts = []; yield
        seller_id_str = "0"
        try:
            full_url = self.router.url
            if full_url and "?" in full_url:
                parsed_url = urlparse(full_url); query_dict = parse_qs(parsed_url.query)
                id_list = query_dict.get("id")
                if id_list: seller_id_str = id_list[0]
        except Exception: pass
        try: seller_id_int = int(seller_id_str)
        except (ValueError, TypeError): seller_id_int = 0
        if seller_id_int > 0:
            with rx.session() as session:
                seller_info = session.exec(sqlmodel.select(UserInfo).options(sqlalchemy.orm.joinedload(UserInfo.user)).where(UserInfo.id == seller_id_int)).one_or_none()
                self.seller_page_info = seller_info
                if seller_info:
                    posts = session.exec(sqlmodel.select(BlogPostModel).where(BlogPostModel.userinfo_id == seller_id_int, BlogPostModel.publish_active == True).order_by(BlogPostModel.created_at.desc())).all()
                    temp_posts = []
                    for p in posts:
                        product_dto = ProductCardData.from_orm(p); product_dto.price_cop = p.price_cop
                        temp_posts.append(product_dto)
    ...
    @rx.var
    def base_app_url(self) -> str: return get_config().deploy_url
