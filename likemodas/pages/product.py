...
def skeleton_loader() -> rx.Component:
    """Un esqueleto completo para la cuadrícula de la página del producto."""
    return rx.grid(
...
def product_page() -> rx.Component:
    """Página con una única condición principal para garantizar la carga segura de datos."""
    return rx.container(
        # ✨ EXPLICACIÓN: Esta única condición lo protege todo.
        # Solo se mostrará el contenido real cuando la página esté lista Y los datos existan.
        rx.cond(
            ProductDetailState.is_hydrated & ProductDetailState.product_detail.is_not_none(),
            
            # --- Contenido Real (si la condición es verdadera) ---
            rx.grid(
                rx.image(
            ...
            # --- Esqueleto de Carga (si la condición es falsa) ---
            skeleton_loader()
        ),
