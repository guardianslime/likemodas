# likemodas/likemodas.py (VERSIÓN CORREGIDA Y ROBUSTA)
...
# --- CAMBIO 1: Importar los módulos principales ---
from . import blog, contact, navigation, pages, admin, auth, cart, purchases

# La única importación de estado que es segura aquí es la base.
from .auth.state import SessionState

# La importación de páginas está bien.
...
# --- CAMBIO 2: Referenciar los eventos on_load a través de sus módulos ---
...
app.add_page(verification_page, route="/verify-email", on_load=auth.verify_state.VerifyState.verify_token)
app.add_page(forgot_password_page, route="/forgot-password")
app.add_page(reset_password_page, route="/reset-password", on_load=auth.reset_password_state.ResetPasswordState.on_load_check_token)
...
app.add_page(purchase_history_page, route="/my-purchases", title="Mis Compras", on_load=purchases.state.PurchaseHistoryState.load_purchases)

# --- Páginas Privadas de Administración ---
app.add_page(blog.blog_post_list_page, route=navigation.routes.BLOG_POSTS_ROUTE, on_load=blog.state.BlogPostState.load_posts)
app.add_page(blog.blog_post_detail_page, route=f"{navigation.routes.BLOG_POSTS_ROUTE}/[blog_id]", on_load=blog.state.BlogPostState.get_post_detail)
app.add_page(blog.blog_post_add_page, route=navigation.routes.BLOG_POST_ADD_ROUTE)
app.add_page(blog.blog_post_edit_page, route="/blog/[blog_id]/edit", on_load=blog.state.BlogEditFormState.on_load_edit)
app.add_page(admin_confirm_page, route="/admin/confirm-payments", title="Confirmar Pagos", on_load=admin.state.AdminConfirmState.load_pending_purchases)
app.add_page(payment_history_page, route="/admin/payment-history", title="Historial de Pagos", on_load=admin.state.PaymentHistoryState.load_confirmed_purchases)

# Páginas de Contacto
app.add_page(contact.contact_page, route=navigation.routes.CONTACT_US_ROUTE)
app.add_page(contact.contact_entries_list_page, route=navigation.routes.CONTACT_ENTRIES_ROUTE, on_load=contact.state.ContactState.load_entries)