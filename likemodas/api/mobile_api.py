...
async def mobile_login(creds: LoginRequest, session: Session = Depends(get_session)):
    ...
    try:
        ...
        new_session = LocalAuthSession(user_id=user.id, session_id=secure_token, expiration=datetime.now(timezone.utc) + timedelta(days=30))
...
async def get_mobile_profile(user_id: int, session: Session = Depends(get_session)):
    try:
        ...
        return ProfileDTO(username=local_user.username, email=user_info.email, phone=user_info.phone or "", avatar_url=avatar)
...
async def delete_account(user_id: int, req: DeleteAccountRequest, session: Session = Depends(get_session)):
    ...
    user_info.seller_address = None; user_info.seller_city = None; user_info.seller_barrio = None
    ...
    for p in purchases:
        p.shipping_name = "Dato Eliminado"; p.shipping_address = "Dato Eliminado"; p.shipping_phone = None
        session.add(p)
    session.commit()
...
async def create_address(user_id: int, req: CreateAddressRequest, session: Session = Depends(get_session)):
    ...
    new_addr = ShippingAddressModel(userinfo_id=user_id, name=req.name, phone=req.phone, city=req.city, neighborhood=req.neighborhood, address=req.address, is_default=is_def)
...
async def get_products_for_mobile(
    ...
    for p in products:
        ...
        # --- LÓGICA CORREGIDA DE ENVÍO COMBINADO ---
        # 1. Empezamos en False (Restrictivo por defecto para que no se muestre a todos)
        is_combined_eligible = False
        
        # 2. Solo evaluamos si el producto tiene el beneficio activo en base de datos
        if p.combines_shipping and p.userinfo:
            seller_cities = p.userinfo.combined_shipping_cities or []
            
            # 3. CORRECCIÓN CRÍTICA: Si el vendedor NO seleccionó ciudades (lista vacía),
            # asumimos que el beneficio SOLO aplica en SU ciudad de origen.
            if not seller_cities:
                if p.userinfo.seller_city:
                    seller_cities = [p.userinfo.seller_city]
            
            # 4. Normalizamos listas para comparar (ignorando mayúsculas/espacios)
            # Usamos la función normalize_text_api si está disponible, o .strip().lower()
            seller_cities_norm = [c.strip().lower() for c in seller_cities]
            
            # 5. Verificación Final:
            # Solo mostramos la etiqueta si:
            # A) Tenemos la ciudad del comprador (normalized_buyer_city no está vacía)
            # B) La ciudad del comprador está en la lista permitida
            if normalized_buyer_city and normalized_buyer_city in seller_cities_norm:
...
async def get_product_detail(product_id: int, user_id: Optional[int] = None, session: Session = Depends(get_session)):
    try:
        ...
        # 2. Obtener la ciudad del comprador (si está logueado)
        buyer_city = None
        normalized_buyer_city = ""
        ...
        # --- 🛠️ LÓGICA BLINDADA DE MODA COMPLETA 🛠️ ---
        is_moda_eligible = p.is_moda_completa_eligible
        
        if is_moda_eligible and p.userinfo:
            moda_cities = getattr(p.userinfo, 'moda_completa_cities', []) or []
            # Si el vendedor definió ciudades específicas, validamos
            if moda_cities and normalized_buyer_city:
                normalized_moda_cities = [c.strip().lower() for c in moda_cities]
                if normalized_buyer_city not in normalized_moda_cities:
                    is_moda_eligible = False

        # --- 🛠️ LÓGICA CORREGIDA DE ENVÍO COMBINADO 🛠️ ---
        # 1. Empezamos en False (Restrictivo por defecto)
        is_combined_eligible = False 

        if p.combines_shipping and p.userinfo:
            # Obtenemos la configuración del vendedor (si es None, usamos lista vacía)
            combined_cities = getattr(p.userinfo, 'combined_shipping_cities', []) or []

            # 2. REGLA CRÍTICA: Si la lista está vacía, asumimos SOLO la ciudad del vendedor
            if not combined_cities:
                if p.userinfo.seller_city:
                    combined_cities = [p.userinfo.seller_city]
            
            # 3. Normalizamos la lista de ciudades permitidas
            normalized_cities = [c.strip().lower() for c in combined_cities]

            # 4. Verificación Final:
            # Solo es True si el comprador TIENE ciudad Y esa ciudad coincide
            if normalized_buyer_city and normalized_buyer_city in normalized_cities:
                is_combined_eligible = True
        # ----------------------------------------------------
        ...
        variants_dto = []
        if p.variants:
            # Extraer configuración de lightbox de la primera variante si existe
        ...
        # 5. Información de Envío y Vendedor
        ...
        # 6. Estado del Usuario (Guardado, Comprado)
        is_saved = False
        can_review = False
        ...
        return ProductDetailDTO(
            ...
            # --- VALORES BOOLEANOS CALCULADOS ---
            is_moda_completa=is_moda_eligible, 
            combines_shipping=is_combined_eligible, # <--- ¡Aquí está la corrección aplicada!
            # ------------------------------------
...
async def create_product_review(productId: int, body: ReviewSubmissionBody, session: Session = Depends(get_session)):
    ...
    if existing:
        ...
        post.quality_score = new_score; post.last_interaction_at = datetime.now(timezone.utc)
        session.add(post)
        new_update = CommentModel(userinfo_id=user_info.id, blog_post_id=productId, rating=body.rating, content=body.comment, author_username=user_info.user.username, author_initial=user_info.user.username[0].upper(), parent_comment_id=existing.id, purchase_item_id=existing.purchase_item_id)
        session.add(new_update); session.commit()
        ...
        post.quality_score = new_score; post.last_interaction_at = datetime.now(timezone.utc)
        session.add(post)
        new_comment = CommentModel(userinfo_id=user_info.id, blog_post_id=productId, rating=body.rating, content=body.comment, author_username=user_info.user.username, author_initial=user_info.user.username[0].upper(), purchase_item_id=has_bought)
        session.add(new_comment); session.commit()
        return {"message": "Opinión creada"}

@router.post("/reports")
async def create_report(req: ReportRequest, user_id: int = Query(...), session: Session = Depends(get_session)):
    try:
        user_info = get_user_info(session, user_id)
        new_report = ReportModel(reporter_id=user_info.id, reason=req.reason, status=ReportStatus.PENDING, blog_post_id=req.target_id if req.target_type == "post" else None, comment_id=req.target_id if req.target_type == "comment" else None)
        session.add(new_report)
        admins = session.exec(select(UserInfo).where(UserInfo.role == UserRole.ADMIN)).all()
        for admin in admins:
            note = NotificationModel(userinfo_id=admin.id, message=f"🚨 Nuevo reporte de {req.target_type}: {req.reason[:30]}...", url="/admin/reports")
            session.add(note)
        session.commit()
...
async def calculate_cart(user_id: int, req: CartCalculationRequest, session: Session = Depends(get_session)):
    ...
    buyer_city = default_addr.city if default_addr else None; buyer_barrio = default_addr.neighborhood if default_addr else None
    ...
    subtotal_base = 0.0; items_for_shipping = []
    ...
    subtotal_con_iva = subtotal_base; free_shipping_achieved = False
    ...
    if not free_shipping_achieved and default_addr:
        ...
        for seller_id, items_from_seller in seller_groups.items():
            combinable_items = [p for p in items_from_seller if p.combines_shipping]; individual_items = [p for p in items_from_seller if not p.combines_shipping]
            seller_data = seller_data_map.get(seller_id); seller_city = seller_data.get("city") if seller_data else None; seller_barrio = seller_data.get("barrio") if seller_data else None
...
async def checkout(user_id: int, req: CheckoutRequest, session: Session = Depends(get_session)):
    ...
    db_posts = session.exec(select(BlogPostModel).where(BlogPostModel.id.in_(product_ids))).all()
    post_map = {p.id: p for p in db_posts}
    subtotal_base = 0.0; items_to_create = []; seller_groups = defaultdict(list); buyer_city = address.city; buyer_barrio = address.neighborhood
    for item in req.items:
        post = post_map.get(item.product_id)
    ...
    subtotal_con_iva = subtotal_base; free_shipping = False
    ...
    if not free_shipping:
        ...
        for uid, products in seller_groups.items():
            seller = seller_map.get(uid); s_city = seller.seller_city if seller else None; s_barrio = seller.seller_barrio if seller else None
            combinables = [x["post"] for x in products if x["post"].combines_shipping]; individuales = [x["post"] for x in products if not x["post"].combines_shipping]
    ...
    if req.payment_method == "Online":
        try:
            ...
            else: logger.error(f"Fallo al generar link Wompi para compra {new_purchase.id}")
            ...
            try:
                    ...
                    for item in p.items:
                        ...
                        if item.blog_post:
                            ...
                            try:
                                ...
                                if variant_img: img_path = get_full_image_url(variant_img)
                                else: img_path = extract_display_image(item.blog_post)
                                img = img_path
                            except Exception: img = "" 
                        
                        variant_str = ""
                        if item.selected_variant and isinstance(item.selected_variant, dict):
                ...
                if status_val == PurchaseStatus.PENDING_PAYMENT.value and p.payment_method == "Online":
                    ...
                    if p.estimated_delivery_date:
                        ...
                        except Exception: tracking_msg = "Tu pedido está en camino."
                    else: tracking_msg = "Tu pedido llegará pronto."
...
async def create_support_ticket(req: CreateTicketRequest, user_id: int = Query(..., alias="user_id"), session: Session = Depends(get_session)):
    ...
    if purchase.items:
        for item in purchase.items:
            if item.blog_post: seller_id = item.blog_post.userinfo_id; break
    if not seller_id: 
...
async def get_invoice(purchase_id: int, user_id: int, session: Session = Depends(get_session)):
    ...
    if purchase.items:
        for item in purchase.items:
            if item.blog_post and item.blog_post.userinfo_id == user_info.id: is_seller = True; break
