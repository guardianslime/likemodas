...
class PurchaseHistoryDTO(BaseModel):
    ...
    # Campos de seguimiento y UI Web
    estimated_delivery: Optional[str] = None
    can_confirm_delivery: bool = False
    tracking_message: Optional[str] = None
    retry_payment_url: Optional[str] = None
    invoice_url: Optional[str] = None        # NUEVO: Para imprimir factura
    can_return: bool = False                 # NUEVO: Para botón de devolución
...
# --- GET MOBILE PURCHASES (ACTUALIZADO) ---
...
async def get_mobile_purchases(user_id: int, session: Session = Depends(get_session)):
    try:
        ...
        for p in purchases:
            ...
            estimated_str = None
            can_confirm = False
            can_return = False # NUEVO FLAG
            tracking_msg = None
            retry_url = None
            invoice_url = None # NUEVO CAMPO
            ...
            try:
                if p.status == PurchaseStatus.SHIPPED:
                    ...
                    if p.estimated_delivery_date:
                     ...
                     can_return = True # Habilitar devolución
                     invoice_url = f"{BASE_URL}/invoice?id={p.id}" # URL de la factura
            ...
            history.append(PurchaseHistoryDTO(
                ...
                invoice_url=invoice_url, # Enviamos la URL
                can_return=can_return    # Enviamos el flag
            ))
        return history
    except Exception as e:
