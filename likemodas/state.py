# likemodas/state.py (VERSIÓN FINAL CON LÓGICA DE DIÁLOGO CORREGIDA)
...
class ProductCardData(rx.Base):
    id: int; title: str; price: float = 0.0; image_urls: list[str] = []; average_rating: float = 0.0; rating_count: int = 0
    @property
    def price_cop(self) -> str: return format_to_cop(self.price)

class AdminPurchaseCardData(rx.Base):
    id: int; customer_name: str; customer_email: str; purchase_date_formatted: str; status: str; total_price: float; shipping_name: str; shipping_full_address: str; shipping_phone: str; items_formatted: list[str]
    @property
    def total_price_cop(self) -> str: return format_to_cop(self.total_price)
        
class UserPurchaseHistoryCardData(rx.Base):
    id: int; purchase_date_formatted: str; status: str; total_price_cop: str; shipping_name: str; shipping_address: str; shipping_neighborhood: str; shipping_city: str; shipping_phone: str; items_formatted: list[str]
...
class AppState(reflex_local_auth.LocalAuthState):
    ...
    def my_userinfo_id(self) -> str | None: 
        if self.authenticated_user_info is None: return None
    ...
    def handle_registration_email(self, form_data: dict):
        ...
        try:
            with rx.session() as session:
                ...
                new_user = LocalUser(username=username, email=email, password_hash=bcrypt.hashpw(password.encode("utf-8"), bcrypt.gensalt()).decode("utf-8"), enabled=True)
            ...
            self.error_message = f"Error inesperado: {e}"; print(f"Error: {e}")
    message: str = ""; is_verified: bool = False
    @rx.event
    def verify_token(self):
        token = self.router.page.query_params.get("token", "")
        if not token: self.message = "Token no proporcionado."; return
        with rx.session() as session:
            db_token = session.exec(sqlmodel.select(VerificationToken).where(VerificationToken.token == token)).one_or_none()
            is_expired = not db_token or datetime.now(timezone.utc) > db_token.expires_at.replace(tzinfo=timezone.utc)
            if is_expired:
            ...
            if user_info:
                user_info.is_verified = True; session.add(user_info); session.delete(db_token); session.commit()
    ...
    def handle_forgot_password(self, form_data: dict):
        ...
        if not self.email: self.message, self.is_success = "Introduce tu correo.", False; return
        with rx.session() as session:
            user_info = session.exec(sqlmodel.select(UserInfo).where(UserInfo.email == self.email)).one_or_none()
            if user_info:
                token_str = secrets.token_urlsafe(32); expires = datetime.now(timezone.utc) + timedelta(hours=1)
        ...
        self.message, self.is_success = "Si el correo existe, se ha enviado un enlace.", True
    def on_load_check_token(self):
        self.token = self.router.page.query_params.get("token", "")
        if not self.token: self.message, self.is_token_valid = "Token no válido.", False; return
        with rx.session() as session:
            db_token = session.exec(sqlmodel.select(PasswordResetToken).where(PasswordResetToken.token == self.token)).one_or_none()
            is_expired = not db_token or datetime.now(timezone.utc) > db_token.expires_at.replace(tzinfo=timezone.utc)
            if is_expired:
                self.message, self.is_token_valid = "Enlace inválido o expirado.", False
                if db_token: session.delete(db_token); session.commit()
                return
    ...
    def handle_reset_password(self, form_data: dict):
        ...
        with rx.session() as session:
            db_token = session.exec(sqlmodel.select(PasswordResetToken).where(PasswordResetToken.token == self.token)).one_or_none()
            if not db_token: self.message = "Token ya utilizado."; return
            user = session.get(LocalUser, db_token.user_id)
            if user:
                user.password_hash = bcrypt.hashpw(self.password.encode("utf-8"), bcrypt.gensalt()); session.add(user); session.delete(db_token); session.commit()
                yield rx.toast.success("¡Contraseña actualizada!")
                return rx.redirect(reflex_local_auth.routes.LOGIN_ROUTE)

    # --- UI STATE / ESTADO DE LA INTERFAZ ---
    show_notifications: bool = False
    post_to_delete: Optional[BlogPostModel] = None

    def toggle_notifications(self): self.show_notifications = not self.show_notifications

    @rx.event
    def show_delete_dialog(self, post_id: int):
        """Busca el post y lo pone en el estado para mostrar el diálogo."""
        with rx.session() as session:
            self.post_to_delete = session.get(BlogPostModel, post_id)
    ...
    search_tipo_prenda: str = ""; search_tipo_zapato: str = ""; search_tipo_mochila: str = ""; search_tipo_general: str = ""; search_color: str = ""; search_talla: str = ""; search_numero_calzado: str = ""; search_material_tela: str = ""; search_medida_talla: str = ""
    ...
    def add_to_cart(self, post_id: int):
        ...
        return rx.toast.success("Producto añadido.")
    @rx.event
    def remove_from_cart(self, post_id: int):
        if post_id in self.cart and self.cart[post_id] > 0: self.cart[post_id] -= 1
        if self.cart.get(post_id, 0) <= 0: del self.cart[post_id]
    @rx.event
    def on_load(self):
    ...
    def _clear_add_form(self): self.title, self.content, self.price, self.category, self.tipo_prenda, self.temp_images = "", "", "", "", "", []
    def _create_post_from_state(self, is_published: bool) -> BlogPostModel:
        ...
        return BlogPostModel(userinfo_id=self.authenticated_user_info.id, title=self.title, content=self.content, price=price_float, image_urls=self.temp_images, category=self.category, attributes=attributes, publish_active=is_published, publish_date=datetime.now(timezone.utc) if is_published else None)
    @rx.event
    def submit_draft(self):
        ...
        self._clear_add_form(); yield rx.toast.success("Borrador guardado.")
        return rx.redirect(navigation.routes.BLOG_POSTS_ROUTE)
    @rx.event
    def submit_and_publish(self):
        if not self.is_admin or not all([self.title, self.price, self.category]): return rx.toast.error("Título, precio y categoría son obligatorios.")
        with rx.session() as session:
    ...
    def current_image_url(self) -> str:
        if self.post and self.post.image_urls: return self.post.image_urls[self.current_image_index % len(self.post.image_urls)]
        return ""
    def next_image(self):
        if self.post and self.post.image_urls: self.current_image_index = (self.current_image_index + 1) % len(self.post.image_urls)
    def prev_image(self):
        if self.post and self.post.image_urls: self.current_image_index = (self.current_image_index - 1 + len(self.post.image_urls)) % len(self.post.image_urls)
    @rx.event
    def on_load_public_detail(self):
        self.current_image_index = 0; self.is_post_loading = True; self.post = None; yield
        pid = self.router.page.params.get("id", 0)
        if pid == 0: self.is_post_loading = False; return
        with rx.session() as session:
            ...
            if db_post: self.post = db_post; self.comments = sorted(db_post.comments, key=lambda c: c.created_at, reverse=True)
            else: self.comments = []
    ...
    def add_new_address(self, form_data: dict):
        if not all([form_data.get("name"), form_data.get("phone"), self.city, form_data.get("address")]): return rx.toast.error("Campos requeridos.")
        with rx.session() as session:
        ...
        self.show_form = False; yield self.load_addresses(); return rx.toast.success("Dirección guardada.")
    @rx.event
    ...
    def delete_address(self, address_id: int):
        ...
        with rx.session() as session:
            address_to_delete = session.exec(sqlmodel.select(ShippingAddressModel).where(ShippingAddressModel.id == address_id, ShippingAddressModel.userinfo_id == self.authenticated_user_info.id)).one_or_none()
            if address_to_delete:
                session.delete(address_to_delete); session.commit(); yield self.load_addresses(); return rx.toast.success("Dirección eliminada.")
            return rx.toast.error("No se pudo eliminar.")
    @rx.event
    ...
    def handle_checkout(self):
        ...
        if not self.authenticated_user_info: return rx.toast.error("Error de usuario.")
        with rx.session() as session:
            ...
            session.commit()
        self.cart.clear(); self.default_shipping_address = None; yield self.notify_admin_of_new_purchase
        yield rx.toast.success("¡Gracias por tu compra!")
        return rx.redirect("/my-purchases")
        
    # --- PANEL DE ADMINISTRACIÓN ---
    pending_purchases: List[AdminPurchaseCardData] = rx.Field(default_factory=list); confirmed_purchases: List[AdminPurchaseCardData] = rx.Field(default_factory=list); new_purchase_notification: bool = False; search_query_admin_posts: str = ""; all_users: List[UserInfo] = []
    ...
    def load_pending_purchases(self):
        ...
        with rx.session() as session:
            ...
            self.pending_purchases = [AdminPurchaseCardData.from_orm(p, update={'customer_name': p.userinfo.user.username, 'customer_email': p.userinfo.email, 'shipping_full_address': f"{p.shipping_address}, {p.shipping_neighborhood}, {p.shipping_city}"}) for p in results]
    ...
    def load_confirmed_purchases(self):
        ...
        with rx.session() as session:
            ...
            self.confirmed_purchases = [AdminPurchaseCardData.from_orm(p, update={'customer_name': p.userinfo.user.username, 'customer_email': p.userinfo.email, 'shipping_full_address': f"{p.shipping_address}, {p.shipping_neighborhood}, {p.shipping_city}"}) for p in results]
    
    # --- GESTIÓN DE BLOG (ADMIN) ---
    admin_posts: List[BlogPostModel] = rx.Field(default_factory=list); post_title: str = ""; post_content: str = ""; price_str: str = ""
    @rx.var
    ...
    def load_admin_posts(self):
        if not self.is_admin: self.admin_posts = []; return
        with rx.session() as session: self.admin_posts = session.exec(sqlmodel.select(BlogPostModel).order_by(BlogPostModel.created_at.desc())).all()
    @rx.event
    def get_post_detail(self):
        pid = getattr(self, 'blog_id', None)
        if pid:
            with rx.session() as session: self.post = session.get(BlogPostModel, pid)
    @rx.event
        ...
        if pid:
            with rx.session() as session:
                ...
                if db_post and db_post.userinfo_id == self.authenticated_user_info.id:
                    self.post, self.post_title, self.post_content, self.price_str = db_post, db_post.title, db_post.content, str(db_post.price)
                else:
                    yield rx.toast.error("No tienes permiso para editar."); yield rx.redirect("/")
    @rx.event
    def handle_edit_submit(self, form_data: dict):
        ...
        with rx.session() as session:
            ...
            if post_to_update and post_to_update.userinfo_id == self.authenticated_user_info.id:
                ...
                except (ValueError, TypeError): return rx.toast.error("Precio inválido.")
                session.add(post_to_update); session.commit()
                yield rx.toast.success("Post actualizado."); return rx.redirect(f"/blog-public/{self.post.id}")
            else: return rx.toast.error("Acción no permitida.")
    ...
    def toggle_publish_status(self, post_id: int):
        ...
        with rx.session() as session:
            ...
            if post_to_update and post_to_update.userinfo_id == self.authenticated_user_info.id:
                ...
                session.add(post_to_update); session.commit()
                yield self.get_post_detail(); yield rx.toast.info(f"Estado cambiado a: {'Activo' if post_to_update.publish_active else 'Inactivo'}")
            else: yield rx.toast.error("No tienes permiso.")
                
    # --- HISTORIAL DE COMPRAS (USUARIO) ---
    user_purchases: List[UserPurchaseHistoryCardData] = rx.Field(default_factory=list); search_query_user_history: str = ""
    ...
    def load_purchases(self):
        ...
        with rx.session() as session:
            ...
            self.user_purchases = [UserPurchaseHistoryCardData.from_orm(p) for p in results]

    # --- CONTACTO ---
    form_data: dict = {}; did_submit_contact: bool = False; contact_entries: list[ContactEntryModel] = rx.Field(default_factory=list); search_query_contact: str = ""
    ...
    def thank_you_message(self) -> str:
        ...
        return f"¡Gracias, {first_name}!" if first_name else "¡Gracias!"
    @rx.var
    ...
    # --- BÚSQUEDA ---
    search_term: str = ""; search_results: List[ProductCardData] = rx.Field(default_factory=list)
    ...
    def ban_user(self, userinfo_id: int, days: int = 7):
        ...
        with rx.session() as session:
            ...
            if user_info:
                ...
                user_info.is_banned = True; user_info.ban_expires_at = datetime.now(timezone.utc) + timedelta(days=days)
                session.add(user_info); session.commit()
        return self.load_all_users()
    @rx.event
