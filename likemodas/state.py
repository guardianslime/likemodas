...
# Esto es vital: La lista 'updates' contiene objetos del mismo tipo
...
def calculate_rating(session: Session, product_id: int):
    """
    Calcula el rating real: Solo cuenta 1 voto por usuario (compra) usando
    la actualización más reciente.
    """
    # 1. Traer solo los comentarios PADRE (raíz del hilo)
    ...
    for parent in parent_comments:
        # 2. Si tiene actualizaciones, la calificación válida es la del hijo más reciente
        if parent.updates:
            ...
            # 3. Si no hay actualizaciones, vale la del padre
...
async def get_product_detail(product_id: int, user_id: Optional[int] = None, session: Session = Depends(get_session)):
    try:
        ...
        # 1. Traer solo comentarios PADRE (raíz)
        ...
        for parent in db_parent_reviews:
            updates_dtos = []
            if parent.updates:
                # 2. Ordenar y crear DTOs para las actualizaciones (Hijos)
            ...
            # 3. Crear DTO para el Padre, incluyendo la lista de hijos
            reviews_list.append(ReviewDTO(
                ...
                updates=updates_dtos # <--- AQUÍ SE PASA LA LISTA AL APP
            ))
            
        avg_rating, rating_count = calculate_rating(session, p.id)

        is_saved = False # Inicialización para evitar error
        can_review = False
...
async def create_product_review(product_id: int, body: ReviewSubmissionBody, session: Session = Depends(get_session)):
    ...
    # 2. Verificar si ya existe una opinión PADRE para esta compra
    ...
    if existing_comment:
        # Lógica de Actualización (Crear Hijo vinculado)
