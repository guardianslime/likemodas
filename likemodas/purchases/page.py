# likemodas/purchases/page.py (VERSIÓN FINAL)
import reflex as rx
...
# --- CAMBIO CLAVE #1: La función ahora acepta un rx.Var[dict] ---
def purchase_item_card(item: rx.Var[dict]) -> rx.Component:
    """
    Una tarjeta visual para un solo artículo comprado.
    Ahora lee los datos desde un diccionario.
    """
    return rx.box(
        rx.vstack(
            rx.box(
                rx.cond(
                    # Accedemos a los datos con corchetes: item["image_url"]
                    item["image_url"] != "",
                    rx.image(
                    ...
                    rx.box(
                        rx.icon("image_off", size=32), width="100%", height="130px", 
                        bg=rx.color("gray", 3), display="flex", align_items="center", justify_content="center"
                    )
                ),
                height="130px", width="100%", border_radius="md", overflow="hidden"
            ),
            rx.vstack(
                ...
                spacing="1", align_items="start", width="100%", padding_x="0.2em"
            ),
            spacing="2", align_items="stretch"
        ),
        # El on_click también usa corchetes para obtener el id
        on_click=lambda: AppState.open_product_detail_modal(item["id"]),
        cursor="pointer", padding="0.5em", border_radius="lg", border="1px solid",
        border_color=rx.color("gray", 6), width="180px",
        _hover={"box_shadow": "0 0 10px rgba(128, 128, 128, 0.5)", "transform": "scale(1.03)"},
        transition="all 0.2s ease-in-out",
    )

def purchase_items_gallery(items: rx.Var[list[dict]]) -> rx.Component:
    """Renderiza la galería de miniaturas. Acepta una lista de diccionarios."""
    return rx.vstack(
        rx.text("Artículos Comprados:", weight="medium", size="4"),
        rx.flex(
            rx.foreach(items, purchase_item_card),
            spacing="4", wrap="wrap", padding_y="0.5em",
        ),
    ...
    page_content = rx.center(
        rx.vstack(
            ...
            rx.cond(
                ...
                rx.foreach(
                    ...
                    lambda purchase: rx.card(
                        rx.vstack(
                            ...
                            # --- CAMBIO CLAVE #2: Se llama a la misma función, pero ahora recibe una lista de diccionarios ---
                ...
                rx.center(
                    rx.text("No se encontraron compras para tu búsqueda."), padding_y="2em",
                )
            ),
