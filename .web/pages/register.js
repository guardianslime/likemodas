...
export function Fragment_87f97b098081b98460cd6c4a62b9bcfd () {
  ...
  const state__local_auth_state__registration_state = useContext(StateContexts.state__local_auth_state__registration_state)
  ...
  const ref_my_navbar_hstack_desktop = useRef(null); refs['ref_my_navbar_hstack_desktop'] = ref_my_navbar_hstack_desktop;
    ...
    const handleSubmit_63fc89db6dac36fa3d15ba4e1e01bff4 = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"email": getRefValue(refs['ref_email']), "confirm_password": getRefValue(refs['ref_confirm_password']), "username": getRefValue(refs['ref_username']), "password": getRefValue(refs['ref_password'])}}

        addEvents([Event("state.local_auth_state.registration_state.my_register_state.handle_registration_email", {form_data:form_data})])
  ...
  <Fragment>
  {isTrue(state__local_auth_state__registration_state.success) ? (
  <Fragment>
  ...
  <Fragment>
  <RadixThemesCard>
  <RadixFormRoot className={`Root`} css={{"width": "100%"}} onSubmit={handleSubmit_63fc89db6dac36fa3d15ba4e1e01bff4}>
  <RadixThemesFlex align={`start`} className={`rx-Stack`} css={{"minWidth": "50vw"}} direction={`column`} gap={`3`}>
  <RadixThemesHeading size={`7`}>
  {`Create an account`}
</RadixThemesHeading>
  <Fragment>
  {isTrue(((state__local_auth_state__registration_state.error_message) !== (""))) ? (
  <Fragment>
  ...
  {state__local_auth_state__registration_state.error_message}
</RadixThemesCallout.Text>
  ...
  <RadixThemesLink css={{"&:hover": {"color": "var(--accent-8)"}}} onClick={(_e) => addEvents([Event("_redirect", {path:`/login`,external:false})], (_e), {})}>
  {`Login`}
</RadixThemesLink>
</RadixThemesFlex>
</RadixThemesFlex>
...
export default function Component() {
  ...
  <Fragment_87f97b098081b98460cd6c4a62b9bcfd/>
  <NextHead>
  <title>
  {`Register`}
</title>
