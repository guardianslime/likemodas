...
from likemodas.models import (
    ...
    SupportTicketModel, SupportMessageModel, ReportModel, ReportStatus # ✅ Importamos ReportModel
)
...
class VariantDTO(BaseModel):
    ...
    images: List[str]
    attributes: Dict[str, Any] = {} # ✅ Importante para los selectores de la App
...
class ProductDetailDTO(BaseModel):
    ...
    # ✅ DATOS CRÍTICOS PARA EL CÁLCULO DE ENVÍO EN LA APP
    shipping_cost: Optional[float] = None
    is_moda_completa: bool
    combines_shipping: bool
    free_shipping_threshold: Optional[float] = None
    shipping_display_text: Optional[str] = None # Texto pre-calculado por si acaso
    ...
    try:
        ...
        for v in safe_variants:
            ...
            variants_dto.append(VariantDTO(
                ...
                attributes=attrs # ✅ ENVÍO DE ATRIBUTOS PARA QUE FUNCIONEN LOS SELECTORES
            ))
...
# ... (Resto del archivo sin cambios: checkout, notifications, etc.)
...
async def mobile_checkout(user_id: int, req: CheckoutRequest, session: Session = Depends(get_session)):
    # ... código de checkout (asumo que está en tu versión local, si no, es el estándar) ...
