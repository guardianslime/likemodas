...
# (Esta función es para la página /add, pero la dejamos aquí ya que el
# archivo 'forms.py' a veces se usa para ambos. El código es idéntico
# al de 'add.py'.)
# =============================================================================
def blog_post_add_form() -> rx.Component:
    ...
    def attributes_and_stock_section() -> rx.Component:
        ...
        ropa_attributes = rx.vstack(
            rx.text("Talla"),
            rx.hstack(
                # --- CAMBIO: Usamos searchable_select en lugar de rx.select ---
                searchable_select(
                    placeholder="Buscar talla...",
                    options=AppState.filtered_attr_tallas_ropa,
                    on_change_select=AppState.set_temp_talla,
                    value_select=AppState.temp_talla,
                    search_value=AppState.search_attr_talla_ropa,
                    on_change_search=AppState.set_search_attr_talla_ropa,
                    filter_name="talla_ropa_filter",
                ),
                # ------------------------------------------------------------
                rx.button(
                    "Añadir", 
                    on_click=lambda: AppState.add_variant_attribute("Talla", AppState.temp_talla), 
                    type="button"
                ),
                align_items="start", # Alineación para que se vea bien con el buscador
                width="100%"
            ),
            rx.flex(
        ...
        calzado_attributes = rx.vstack(
            ...
            rx.hstack(
                ...
                searchable_select(
                    ...
                    on_change_select=AppState.set_temp_numero,
                    value_select=AppState.temp_numero,
                    search_value=AppState.search_attr_numero_calzado,
                    on_change_search=AppState.set_search_attr_numero_calzado,
                    filter_name="numero_calzado_filter",
                ),
                # ------------------------------------------------------------
                rx.button(
                    "Añadir", 
                    on_click=lambda: AppState.add_variant_attribute("Número", AppState.temp_numero), 
                    type="button"
                ),
                align_items="start", 
                width="100%"
    ...
    def image_and_group_section() -> rx.Component:
        ...
        unassigned_images_display = rx.vstack(
            ...
            rx.button(
                ...
                width="100%", 
                type="button",
                is_disabled=AppState.edit_image_selection_for_grouping.length() == 0
            ),
            spacing="3", width="100%", align_items="stretch",
        )
                                ...
                                rx.foreach(
                                    ...
                                    lambda image_url: rx.box(
                                        ...
                                        rx.icon_button(
                                            ...
                                            variant="solid",
                                            size="1",
                                            on_click=AppState.remove_image_from_edit_group(index, image_url),
                                            style={
                                                "position": "absolute", 
                                                "top": "-5px", 
                                                "right": "-5px",
        ...
        return rx.vstack(
            ...
            unassigned_images_display, # Sección para imágenes no asignadas
            
            rx.cond(
                AppState.edit_variant_groups.length() > 0, 
                integrated_group_and_image_selector
            ),
