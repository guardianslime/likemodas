...
async def get_products_for_mobile(
    ...
    for p in products:
        ...
        if p.userinfo and is_moda_eligible:
            # Usamos getattr por seguridad si falla el modelo
            seller_moda_cities = getattr(p.userinfo, 'moda_completa_cities', []) or []
        ...
        if p.userinfo and is_combined_eligible:
            # Usamos getattr para seguridad absoluta
            seller_combined_cities = getattr(p.userinfo, 'combined_shipping_cities', []) or []
            
            # Solo filtramos si el vendedor restringió ciudades y tenemos ciudad de comprador
            if seller_combined_cities and normalized_buyer_city:
                ...
                # Si la ciudad del comprador NO está en la lista permitida -> SE OCULTA (False)
        ...
        result.append(ProductListDTO(
            ...
            combines_shipping=is_combined_eligible, # <--- Variable corregida
    ...
    try:
        ...
        if is_moda_eligible and p.userinfo and moda_cities:
            if buyer_city:
                ...
                normalized_moda = [c.strip().lower() for c in moda_cities]
                if normalized_buyer not in normalized_moda:
                    is_moda_eligible = False
        ...
        if is_combined_eligible and p.userinfo and combined_cities:
            if buyer_city:
                ...
                # Opcional: Ocultar si no sabemos dónde vive el usuario
                pass
        ...
        return ProductDetailDTO(
            ...
            # --- 🛠️ CORRECCIÓN AQUÍ: USAR LA VARIABLE FILTRADA 🛠️ ---
            is_moda_completa=is_moda_eligible, 
...
# ... (El resto del archivo no requiere cambios lógicos, pero lo incluyo para integridad) ...
