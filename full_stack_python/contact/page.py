...
def contact_form() -> rx.Component:
    ...
    return rx.form.root(
        ...
        reset_on_submit=True, # Resetea los campos del formulario visualmente
    )
...
def contact_page() -> rx.Component:
    ...
    return base(
        rx.vstack(
            ...
            # El on_load se encarga de redirigir si el usuario no está logueado.
            # Este rx.cond asegura que la UI cambie fluidamente.
            rx.cond(
                ...
                # Mensaje mientras se redirige al login
                rx.vstack(
    ...
    return rx.table.root(
        rx.table.header(
            rx.table.row(
                ...
                rx.table.column_header_cell("Enviado por"), # Nueva columna
            )
        ),
        rx.table.body(
            rx.foreach(
                ...
                lambda entry: rx.table.row(
                    ...
                    # Muestra el nombre de usuario o "Invitado"
                    rx.table.cell(
                        rx.cond(
                            entry.user,
                            entry.user.username,
...
@rx.page(
    route=navigation.routes.CONTACT_ENTRIES,
    on_load=[ContactState.load_entries, AuthState.check_login]
)
